...
def kruskal_cola_prioridad(grafo):

    #Creamos el conjunto de nodos para pasarselo a la particion
    nodos=set()
    ...
    #Creamos la cola de prioridad y le introducimos todas las aristas con
    heap=[]
    for (nodo1,nodo2),peso in grafo.items():
        heapq.heappush(heap,(peso,nodo1,nodo2))

    #Creamos el arbol resultado
    arbol={}

    #Hasta que la cola de prioridad no tenga ningun elemento
    while heap:

        #Extraemos la que tenga menor peso hasta el momento
        peso,nodo1,nodo2=heapq.heappop(heap)

        #Si el representante de ambos nodos no es el mismo (no están en el mismo conjunto)
        if particion.__getitem__(nodo1)!=particion.__getitem__(nodo2):

            #Añadimos la arista al arbol y unimos ambos conjuntos
